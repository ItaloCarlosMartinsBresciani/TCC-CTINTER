...
<?php
    ...
    if($return['valid']) 
    {
        ...
        echo '<h2 class="py-2">Informações - '.$return['name_person'].' 
                <a href="supervisorEdit.php?type=profile&id='.$idHex.'"> <!--botão de edição -->
    ...
    if($return == null)
    {
        ...
        //-------------------------------------------------------------------------------------------
        //Selecionando o nome do professor e o id da empresa
        $query = "SELECT p.name_person, c.fk_company FROM person p, company_employee c WHERE c.fk_id = ".$id_person." AND c.fk_id = p.id_person";
        ...
        $id_company = $return[0]['fk_company'];
        
        $name_supervisor = $return[0]['name_person'];

        
        //------------------------------------------------------------------------------------------
        //Selecionando o email da empresa
        $query = "SELECT c.email_company FROM company c, company_employee e WHERE c.id_company = ".$id_company." AND c.id_company = e.fk_company";
        ...
        $email_company = $return[0]['email_company'];
        ...
        <div class='modal-dialog'>
            <div class='modal-content'>
                ...
                <form action='../../app/php/email/sendEmailSupervisor.php?type=editionRequest' method='POST'> 
                    <div class='modal-body'>
                        <div class='mb-3'>
                            <label for='message-email' class='col-form-label'>E-mail:</label>
                            <input type='email' class='form-control' id='message-email' name='message-email' value=".$email_company.">
                        </div>
                        <div class='mb-3'>
                        ...
                        <div class='mb-3'>
                            ...
                            <textarea class='form-control' id='message-text' name='message-text' style='height:200px;'>O(A) supervisor(a) ".$name_supervisor." está pedindo permissão para modificar seus dados pessoais. \n Acesse sua conta no Sistema de Controle de Estágio, entre na página HOME e clique no botão de PERMITIR EDIÇÃO para responder ao pedido.</textarea>
                        </div>
                    </div>
