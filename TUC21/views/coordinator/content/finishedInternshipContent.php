<?php 
    ...
    $query = 'SELECT i.id_internship_data, p.name_person, p.telephone_person, p.email_person, p.treatment_person, p.register_date, i.valid, i.finished_date FROM person p, internship_data i, student s WHERE p.valid AND p.id_person = i.fk_student AND s.fk_id = i.fk_student AND s.fk_university ='.$id_university." AND i.finished = TRUE";
    ...
    if($count > 0) {
        ...
        foreach($filterPerson as $key => $person) {
            $date = date('d/m/Y', strtotime($person['finished_date']));
            $idHex = codeId($person['id_internship_data']);
            ...
            <tr>
                ...
                <td>
                    <a href="coordinatorInfo.php?type=internship_finished&id='.$idHex.'" class="btn btn-primary">
        ...
        echo '<div class="col-12 h5 mt-5 text-secondary text-center">Nenhum estágio encontrado</div>';
    }
