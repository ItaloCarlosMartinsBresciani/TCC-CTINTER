...
<head>
    ...
    <?php
      if (isset($_GET['type'])) {
        ...
        if($type == 'person')
        {
            echo "<title>Recuperação de Funcionários- Coordenador</title>";
        }
        else if($type == 'student')
        {
            echo "<title>Recuperação de Alunos- Coordenador</title>";
        } 
      } else {
                    ...
                    <form action="../../app/php/principal/logic/principalEditLogic.php?<?php echo "type=$type&id=$id"?>" method="POST">
                        ...
                        <?php
                            if ($type == 'person') {
                              ...
                              $query = 'SELECT p.id_person, p.name_person, p.telephone_person, p.email_person, p.treatment_person, p.register_date, p.deleted_date, p.valid FROM person p, student s WHERE NOT p.valid AND p.id_person = s.fk_id';
                                ...
                                if(count($return) > 0) {
                                    echo '<h2 class="py-2">Alunos Excluídos</h2>';
                                    ...
                                    echo '<div class="h3 text-center my-5 py-5 text-secondary">Você não tem alunos excluídos</div>';
                                }
    ...
    <div class="modal fade show" id="recycleModal" role="dialog" tabindex="-1" aria-labelledby="recycleModalTitle" aria-hidden="true" onload="teste()">
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <form action="../../app/php/principal/logic/principalRecycleLogic.php" method="GET">
            <div class="modal-body"></div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Fechar</button>
