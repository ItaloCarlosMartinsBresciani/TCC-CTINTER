...
<?php
    ...
    if ($return)
    {
        echo "<a href='../../app/php/professor/logic/changeProfessorInformation.php'><button type='button' class='btn btn-primary'>Entrar como Coordenador</button></a>";
    }

    $query = "SELECT * FROM person p, university_employee u WHERE p.id_person = $id AND fk_id = $id";
    ...
    if($return['valid']) 
    {
        ...
        echo '<h2 class="py-2">Informações - '.$return['name_person'].' 
                <a href="professorEdit.php?type=profile&id='.$idHex.'"> <!--botão de edição -->
    ...
    if($return == null)
    {
        ...
        //-------------------------------------------------------------------------------------------
        //Selecionando o nome do professor e o id da universidade
        ...
        $id_university = $return[0]['fk_university'];
        
        $name_professor = $return[0]['name_person'];
        ...
        $email_principal = $return[0]['email_person'];
        ...
        echo "<div class='modal fade' id='sendEmailModal' tabindex='-1' aria-labelledby='sendEmailModalLabel' aria-hidden='true'>
            <div class='modal-dialog'>
                <div class='modal-content'>
                    ...
                    <form action='../../app/php/email/sendEmailProfessor.php?type=editionRequest' method='POST'> 
                        <div class='modal-body'>
                            <div class='mb-3'>
                                <label for='message-email' class='col-form-label'>E-mail:</label>
                                <input type='email' class='form-control' id='message-email' name='message-email' value=".$email_principal.">
                            </div>
                            <div class='mb-3'>
                            ...
                            <div class='mb-3'>
                                ...
                                <textarea class='form-control' id='message-text' name='message-text' style='height:200px;'>O(A) professor(a) ".$name_professor." está pedindo permissão para modificar seus dados pessoais. \n Acesse sua conta no Sistema de Controle de Estágio, entre na página HOME e clique no botão de PERMITIR EDIÇÃO para responder ao pedido.</textarea>
                            </div>
                        </div>
