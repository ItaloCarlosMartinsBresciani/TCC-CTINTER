...
<div id="person-content">
    <?php
        ...
        //Selecionando todos os estagiários desse orientador
        $query = "SELECT p.*, ip.id_internship_plan, ip.advisor_approval_internship_plan, ip.supervisor_approval_internship_plan, ip.date_internship_plan, s.total_hours_student FROM person p, internship_data i, internship_plan ip, student s 
        WHERE p.id_person = i.fk_student AND p.id_person = s.fk_id AND i.fk_advisor = ".$_SESSION["idUser"]." AND ip.fk_internship_data = i.id_internship_data ORDER BY id_person";
        ...
        if($count > 0) 
        { //se o orientador possui estagiários

            $var = 0;

            foreach($filterPerson as $key => $person) 
            { 
                if($person['advisor_approval_internship_plan'] == null && $person['supervisor_approval_internship_plan'] != null)
                {
                    $var = $var + 1;
                }
            }
                    ...
                    {    
                        if (isset($person['total_hours_student']))
                        {
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="../../app/php/email/sendEmailPlanAdvisor.php?type=approve&id='.$idHex.'" class="btn btn-primary text-white">Sim</a>
                                    <a href="../../app/php/email/sendEmailPlanAdvisor.php?type=disapprove&id='.$idHex.'" class="btn btn-primary text-white">Não</a>
                                </td>
                            </tr>';           
                        }                 
                    }
