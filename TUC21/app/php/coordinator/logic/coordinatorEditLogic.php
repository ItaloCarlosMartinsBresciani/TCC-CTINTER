...
foreach($_POST as $key => $value) {
  ...
  if(!empty($value)) {
    ...
    header('Location: ../../../../views/coordinator/coordinatorEdit.php?type='.$type.'&id='.codeId($idDec));
    exit();
  }
}
...
try
{
  ...
  if ($type == 'person') 
  {
    ...
    else
    {
      ...
      echo "<script>window.location.replace('../../../../views/coordinator/coordinatorEdit.php?type=$type&id=$idHex')</script>";
    }
  } 
  ...
  else if($type == 'internship')
  {
    ...
    //     echo "<script>window.location.replace('../../../../views/coordinator/coordinatorEdit.php?type=$type&id=$id')</script>";
    //     exit();
    // }
    // if(intval($diff2) > 600)  
    // {
    ...
    $query = "UPDATE public.internship_data
                  SET role_internship_data = :role_internship_data, 
                      ...
                      scholarship_value_internship_data = :scholarship_value_internship_data, 
                      ...
                      nature_internship_data = :nature_internship_data                                                 
                  WHERE id_internship_data = :id_internship_data;";

    $stmt = $conn->prepare($query);

    $stmt->bindValue(':id_internship_data', $idDec);
    ...
    if($_POST['scholarship-internship-data'] == 'True')
    {
    ...
    {
      $nulo = 0;
      $stmt->bindValue(':scholarship_value_internship_data', $nulo);
    }
    ...
    $stmt->bindValue(':nature_internship_data', $_POST['nature-internship-data']);
