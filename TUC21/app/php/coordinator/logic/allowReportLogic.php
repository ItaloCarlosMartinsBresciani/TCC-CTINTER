...
try
{
    if ($type == 'allow') 
    {
        try{
        //$today = date('d-m-Y');
        $query = "UPDATE internship_reports SET coordinator_signature_internship_report = now(), 
        ...
        $query = "SELECT ir.*, p.name_person, p.email_person FROM internship_data id, internship_reports ir, person p WHERE ir.id_internship_reports = $idDec AND p.id_person = id.fk_student AND ir.fk_internship_data = id.id_internship_data";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $return = $stmt->fetch(PDO::FETCH_ASSOC); 
        $type_report = $return['type_internship_report'];
        $messageText = "O(a) ".$type_report." foi aprovado(a) por todas as autoridades responsáveis pelo seu estágio (supervisor, orientador e coordernador). Imprima, assine e leve para os mesmos pessoalmente para coletar as devidas assinaturas.";
    ...
    else if ($type == 'deny') 
    {
        ...
        $messageText = "O(a) ".$return['type_internship_report']." foi negado(a) por seu(a) coordenador(a)! \n[Escreva aqui as modificações que o estagiário deve fazer no relatório]";
