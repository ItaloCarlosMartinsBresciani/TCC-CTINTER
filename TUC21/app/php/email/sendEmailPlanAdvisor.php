...
if($type == 'approve')
{
    try 
    {
        //atualizando aprovação do orientador
        $query = "UPDATE internship_plan SET advisor_approval_internship_plan = NOW() WHERE id_internship_plan = :id";
        ...
        //selecionando o nome e email do orientador
        $query= "SELECT id_person, name_person, email_person FROM person WHERE id_person = ".$_SESSION["idUser"];
        ...
        {
            $id = $return['id_person'];
            $name_advisor =  $return['name_person']; 
            $email = $return['email_person'];
        }

        //selecionando o id do estágio
        $query= "SELECT fk_university FROM university_employee WHERE fk_id = ".$id." AND active = TRUE";
        ...
        //selecionando o nome e email do orientador
        $query= "SELECT p.name_person, p.email_person FROM person p, university_employee u WHERE u.fk_university = ".$id_university." AND u.role_university_employee = 'Coordenador' AND u.fk_id = p.id_person";
        ...
        try 
        {
            $messageEmail = $email_coordinator;// orientador que vai receber o relatório
            $messageSubject = "Estagio CTI";
            $mail_body = "Olá coordenador(a) ".$name_coordinator."! \nO Plano de Estágio acabou de ser aprovado pelo Orientador.\nEntre em sua página no Sistema de Controle de Estágios da CTI para visualizar e aprovar este documento. \n\nAtenciosamente, \n".$name_advisor; 
...
else if ($type == 'disapprove')//caso não seja aprovado
{
    try
    {
        ...
        $query = "UPDATE internship_plan SET supervisor_approval_internship_plan = NULL, advisor_approval_internship_plan = NULL, active = FALSE
