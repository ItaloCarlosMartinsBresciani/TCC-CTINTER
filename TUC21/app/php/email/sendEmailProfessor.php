...
if($type == 'invitation')
{
    ...
    //Variáveis
    $idProfessor = $_SESSION['idUser'];
    $privateLink = 'http://localhost/UNESP-Internship/TUC21/views/putStudentInformation.php?key='.$acessKey.'&id_professor='.codeId($idProfessor); // Link único de acesso
...
else if ($type == 'editionRequest')
{
    ...
    else 
    {
        ...
        echo "<script>location= '../../../views/professor/components/profileProfessor.php';</script> ";
        exit();
    } 
}
else if($type == 'invalidate_internship')
{
    ...
    try{   
        ...
        $query = "UPDATE internship_data SET validated_advisor = FALSE, valid = FALSE WHERE id_internship_data = ".$id_internship."";
        ...
        $mail_body = "Os dados recém cadastrados foram invalidados pelo(a) seu(sua) orientador(a), entre na sua conta e adeque-os. \n\nCorreções: digite as devidas correções."; // Sem HTML
        $mail_body = urlencode($mail_body); //transforma os caracteres de string para serem aceitos no URL
    ...
    catch (Exception $ex)
    {
        ...
        echo "<script>alert('Erro na invalidação de dados do estágio!');</script>";
        echo "<script>location= '../../../views/professor/professorInfo.php?type=".$type."&id=".$id_internship."';</script> ";
        echo $ex->getMessage();
        exit();
    }
}
else if($type == 'validate_internship')
{
    ...
    try{            
        //Atualizando dados na tabela de informações de estágio
        $query = "UPDATE internship_data SET validated_advisor = TRUE WHERE id_internship_data = ".$id_internship."";
        ...
        //recuperando email do estagiário
        $query = "SELECT p.name_person FROM person p, internship_data i WHERE i.fk_student = p.id_person AND i.id_internship_data = ".$id_internship."";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $return = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $query = "SELECT p.email_person FROM person p, university_employee u WHERE u.fk_id = p.id_person AND u.fk_university = ".$id_university." AND u.role_university_employee = 'Coordenador'";
        $stmt = $conn->prepare($query);
        $stmt->execute();
        $return = $stmt->fetch(PDO::FETCH_ASSOC);
        ...
        $mail_body = "Os dados recém cadastrados do estagiário ".$name_intern." foram validados pelo(a) orientador(a). \nAcesse sua página no Sistema de Controle de Estágios da CTI para analisá-los."; // Sem HTML
        $mail_body = urlencode($mail_body); //transforma os caracteres de string para serem aceitos no URL
