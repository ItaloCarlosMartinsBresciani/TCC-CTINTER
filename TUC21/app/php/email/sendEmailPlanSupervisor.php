...
if($type == 'approve')
{
    try 
    {
        ...
        $query = "UPDATE internship_plan SET supervisor_approval_internship_plan = NOW() WHERE id_internship_plan = :id";
        ...
        //selecionando o nome e email do supervisor
        ...
        $query= "SELECT p.name_person, p.email_person FROM person p, internship_data i WHERE i.id_internship_data = ".$id_internship_data." AND p.id_person = i.fk_advisor";
        ...
        try 
        {
            $messageEmail = $email_advisor; //email do orientador que vai receber o relatório
            $messageSubject = "Estagio CTI";
            $mail_body = "Olá orientador(a) ".$name_advisor."! \nO Plano de Estágio acabou de ser aprovado pelo Supervisor.\nEntre em sua página no Sistema de Controle de Estágios da CTI para visualizar e aprovar este documento. \n\nAtenciosamente, \n".$name_supervisor; 
...
else if ($type == 'disapprove')//caso não seja aprovado
{
    try
    {
        ...
        $query = "UPDATE internship_plan SET supervisor_approval_internship_plan = NULL, valid = FALSE
