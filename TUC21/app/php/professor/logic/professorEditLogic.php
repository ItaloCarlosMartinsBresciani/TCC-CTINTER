...
foreach($_POST as $key => $value) {
  ...
  if(!empty($value) || $key == 'scholarship-value-internship-data') {
    ...
    echo "<script>window.location.replace('../../../../views/professor/professorEdit.php?type=$type&id=$id')</script>";
    exit();
  }
}
...
try
{
  ...
  if ($type == 'person') 
  {
    $query = "SELECT * from person WHERE cpf_person = :cpf_person AND id_person != :id_person";
    ...
    //Consistência rg
    $query = "SELECT * from person WHERE rg_person = :rg_person AND id_person != :id_person";
    ...
    {
      echo "<script>alert('RG digitado já foi cadastrado! Redigite.');</script>";
      echo "<script>window.location.replace('../../../../views/professor/professorEdit.php?type=$type&id=$id')</script>";
    }
  ...
  {
    //Consistência para ver se existe alguém com esse CPF
    $query = "SELECT * from person WHERE cpf_person = :cpf_person AND id_person != :id_person";
    ...
    if($return_rg == null)
    {
      if ($return_cpf == null)
      {
        if(verifyCPF($_POST['cpf-person']))
        {
          ...
          //pegando id do estágio
          $query = "SELECT id_internship_data from internship_data WHERE fk_student = :id_person AND finished = FALSE";
  ...
  else if($type == 'profile')
  {
    ...
    else
    {
      $idHex = codeId($idDec);
      echo "<script>alert('CPF inválido! Redigite.');</script>";
      echo "<script>window.location.replace('../../../../views/professor/professorEdit.php?type=$type&id=$idHex')</script>";
...
{
  $query = "SELECT * FROM person WHERE rg_person = :rg_person";
  ...
  if($return && $return["id_person"] != $idDec)
  {
    $idHex = codeId($idDec);
    echo "<script>alert('RG já cadastrado! Redigite.');</script>";
    echo "<script>window.location.replace('../../../../views/professor/professorEdit.php?type=$type&id=$idHex')</script>";
    exit();
  }

  $query = "SELECT * FROM person WHERE cpf_person = :cpf_person";
  ...
  {
    $idHex = codeId($idDec);
    echo "<script>alert('CPF já cadastrado! Redigite.');</script>";
    echo "<script>window.location.replace('../../../../views/professor/professorEdit.php?type=$type&id=$idHex')</script>";
    exit();
  }
